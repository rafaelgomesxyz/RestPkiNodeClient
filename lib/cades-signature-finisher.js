...
class CadesSignatureFinisher extends SignatureFinisher {
   ...
   async finish() {
      ...
      if (!this._signature) {
         let response = await this._client.post(`Api/CadesSignatures/${this._token}/Finalize`);
         ...
         let response = await this._client.post(`Api/CadesSignatures/${this._token}/SignedBytes`, request);
