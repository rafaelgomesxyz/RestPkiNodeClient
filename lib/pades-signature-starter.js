...
class PadesSignatureStarter extends SignatureStarter {
   ...
   async _startCommon() {
      ...
      try{
         ...
         if(apiVersion >= 2){
            return await this._client.getRestClient().post('Api/v2/PadesSignatures', request).data;
         }
         return await this._client.getRestClient().post('Api/PadesSignatures', request).data;
      } catch (err) { throw err; }
   }
