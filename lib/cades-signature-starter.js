...
class CadesSignatureStarter extends SignatureStarter {
   ...
   async _startCommon() {
      ...
      try {
         ...
         if(apiVersion >= 3){
            response = await this._client.getRestClient().post('Api/v3/CadesSignatures', request).data;
         } else if(apiVersion >= 2) {
            response = await this._client.getRestClient().post('Api/v2/CadesSignatures', request).data;
         } else {
            response = await this._client.getRestClient().post('Api/CadesSignatures', request).data;
         }
         return response;
      } catch(err) { throw err; }
   }
