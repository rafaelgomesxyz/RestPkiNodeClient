...
class Authentication {
   ...
   async startWithWebPki(securityContextId) {
      ...
      try{
         let response = await this._client.getRestClient().post('Api/Authentications', request).data;
   ...
   async completeWithWebPki(token) {
      try{
         let response = await this._client.getRestClient().post(`Api/Authentications/${token}/Finalize`).data;
