...
class CadesSignatureExplorer extends SignatureExplorer {
   ...
   async open(extractContent = false) {
      ...
      try{
         if (this._dataFileContent){
            ...
            let response = await this._client.getRestClient().post('Api/CadesSignatures/Open', request).data;
   ...
   async _getRequiredHashes() {
      ...
      try{
         let response = await this._client.getRestClient().post('Api/CadesSignatures/RequiredHashes', request).data;
