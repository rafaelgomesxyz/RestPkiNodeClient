...
class RestPkiClient {
    ...
    async _upload(content) {
        ...
        try{
            ...
            let partETags = await this._uploadParts(content, client, blobUri, partSize);
    ...
    async _uploadParts(data, client, blobUri, partSize) {
        ...
        if(dataSize <= partSize){
            ...
            try{
                while(dataSize > 0) {
                    uploadPartHttpResponse = await client.post(`${blobUri}/${partNumber}`, 
                    data.subarray(partSize*partNumber, (partSize > dataSize) ? (partSize*partNumber)+dataSize : (partSize*partNumber)+partSize));
